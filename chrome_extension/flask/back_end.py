...
def classify(link):
	...
	# if('breitbart' in link or 'nra' in link or 'heat' in link or 'wsj' in link):
	# 	return 'RIGHT'
	# elif('huffing' in link or 'jones' in link or 'occupy' in link):
	# 	return 'LEFT'


	page_text = get_text(link)
	#print(page_text)
	#print('wreefefefrefrer')
	#print(page_text)
...
def text_is_conservative(text):
	...
	f.write(text)
	f.close()
	trainingVector=prediction.buildTrainingVector("buffer.txt")
	return prediction.clf.predict(trainingVector)[0]
"""
...
def get_text(link):
	...
	alchemy_language = AlchemyLanguageV1(api_key='9db2fce31fb75dd8dff38c03baf14c9d8cf6fdd5')
